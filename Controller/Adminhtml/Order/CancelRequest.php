...
class CancelRequest extends \Magento\Backend\App\Action
{
    /**
    ...
    public function execute()
    {
        ...
        if(!$requestId || !$transferRequest->getId() || $transferRequest->getStatus()) {
            if($transferRequest->getStatus()) $this->messageManager->addErrorMessage(__('You cannot cancel an approved transfer order request.'));
            else $this->messageManager->addErrorMessage(__('Transfer order request does not exist.'));
            return $resultRedirect->setUrl($this->_redirect->getRefererUrl());
        }
        $transferRequest->setStatus(RequestStatus::CANCELED_STATUS);
        $transferRequest->setAdminUserModifyId($this->helperData->getAdminUserId());
        $transferRequest->save();
        $this->messageManager->addSuccessMessage(__('You have successfully canceled a transfer order request.'));
    ...
    {
        return $this->_authorization->isAllowed('Magezon_TransferOrder::transfer_order_requests_cancel');
    }
}